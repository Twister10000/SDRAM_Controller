...
set SDRAM_CLK	{ \PLL:PLL1|altpll_component|auto_generated|pll1|clk[0]}
...
set_input_delay -min -clock $SDRAM_CLK 2.7 [get_ports sdram_dq*]
#Output HIGH Impedance Time	5.4ns 
set_input_delay -max -clock $SDRAM_CLK 5.4 [get_ports sdram_dq*]

#Controller Interface no timing requirments
set_input_delay -min -clock $SDRAM_CLK 0 [get_ports {reset addr* data* we req}]

set_input_delay -max -clock $SDRAM_CLK 0 [get_ports {reset addr* data* we req}]
...
set_output_delay -min -clock $SDRAM_CLK -0.8 [get_ports sdram_dq*]
...
set_output_delay -max -clock $SDRAM_CLK 0 [get_ports {ack valid q*}]
